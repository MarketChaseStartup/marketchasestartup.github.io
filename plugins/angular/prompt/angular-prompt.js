...
angular.module('cgPrompt').factory('prompt',['$modal','$q',function($modal,$q){
    ...
    var prompt = function(options){
        ...
        var modalInstance = $modal.open({
            templateUrl:'plugins/angular/prompt/angular-prompt.html',
        ...
        modalInstance.opened.then(function(){
            setTimeout(function(){
                ...
                $('.modal-dialog').css({"pointer-events":"all"});
                var lastButton = $('.modal-footer .emb-btn').length-1;
                $('#prompt-button-'+lastButton).focus();
            },300);
        });
